#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include <unistd.h>
#include "lib/structure.h"

/**
 * \file quete_recherche.c
 * \brief Fonctions utilisees dans la quete "recherche".
 * \author Mathilde Mottay, Anais Mottier, Clement Mainguy, Moustapha Tsamarayev
 * \version
 * \date 2020
<<<<<<< Updated upstream
*/

void init_items_recherche(item_t * Tab_items_search, item_t * Tab_Items, int nb_items_urbain){
    int i, num, cpt;

    for(i=0, cpt=0; i<nb_items_urbain; i++){
        num = rand()%nb_items_urbain;
        /*Si l'item peut etre trover d'un hexagone de categorie urbain*/
        if(Tab_Items[num].pc_urban != 0){
            Tab_items_search[cpt] = Tab_Items[num];
            cpt++;
        }
    }
}

int compte_items_urbain(item_t * Tab_Items, int nb_items_available){
    int i, cpt;

    for(i=0, cpt=0; i<nb_items_available; i++){
        if(Tab_Items[i].pc_urban != 0)
            cpt++;
    }
    return cpt;
}


=======
*/


/**
 * \fn int compter_items_urbain(item_t * Tab_Items, int nb_items_available)
 * \brief Compte le nombre d'items trouvables dans un hexagone de catégorie "urbain" (pc_urban > 0).
 * \param item_t * Tab_Items
 * \param int nb_items_available
 * \return Retourne un \a int : nombre de items ayant un pc_urban > 0
*/
int compter_items_urbain(item_t * Tab_Items, int nb_items_available){
    int i, cpt;

    for(i=0, cpt=0; i<nb_items_available; i++){
        if(Tab_Items[i]->pc_urban != 0)
            cpt++;
    }
    return cpt;
}


>>>>>>> Stashed changes
/**
 * \fn void init_Tab_Items_urbain(item_t * Tab_Items_urbain, item_t * Tab_Items, int nb_items_urbain)
 * \brief Initialise un tableau contenant tous les items trouvable dans un hexagone de catégorie "urbain".
 * \param item_t * Tab_Items_urbain
 * \param item_t * Tab_Items
 * \param int nb_items_urbain
 * \return Rien
*/
void init_Tab_Items_urbain(item_t * Tab_Items_urbain, item_t * Tab_Items, int nb_items_urbain){
    int i, num, cpt;

    for(i=0, cpt=0; i<nb_items_urbain; i++){
        num = rand()%nb_items_urbain;
        /*Si l'item peut etre trouver d'un hexagone de categorie urbain*/
        if(Tab_Items[num]->pc_urban != 0)
           Tab_Items_urbain[cpt++] = Tab_Items[num];
    }
}


/**
 * \fn void quete_recherche(perso_t * player, cell_t map[D][D], quete_t * quete, quest_map[6][2], item_t * Tab_Items, int nb_items_available)
 * \brief Accï¿½s ï¿½ la quete "recherche".
 * \details
    Le joueur doit aller ï¿½ un endroit donnï¿½ pour trouver un item et le ramener.
 * \param perso_t * player
 * \param cell_t map[D][D]
 * \param quete_t * quete
 * \param quest_map[6][2];
 * \param item_t * Tab_Items
 * \param int nb_items_available
<<<<<<< Updated upstream
 * \return Retourne un \a int : 0 si le jeu continue et -1 si problï¿½me dans la quete.
*/
int quete_recherche(perso_t * player, cell_t map[D][D], quete_t * quete, int quest_map[6][2], item_t * Tab_Items, int nb_items_available){
    int nb_items_urbain, num, choix, conf;

    /*Si la quete n'est pas encore commencï¿½, ou que le joueur y a deja renoncï¿½*/
    if(quete->recherche.trouve==-1){
        quete->recherche.situation=0;

        nb_items_urbain = compte_items_urbain(Tab_Items, nb_items_available); //Nombre d'item que l'on peut trouver dans les hexagones de categorie urbain
        /*Crï¿½ation d'un tableau contenant les items que l'on peux trouver dans un haxagon de categorie urbain*/
        item_t * Tab_items_search = malloc(nb_items_urbain * sizeof(item_t));
        init_items_recherche(Tab_items_search, Tab_Items, nb_items_urbain);

        /*Generation aleatoire de l'item ï¿½ trouver*/
        num = rand()%nb_items_urbain;
        quete->recherche.wanted = Tab_items_search[num];

        /*Description de la situation*/
        printf("Vous croiser la chemin d'un homme qui paraï¿½t sympathique. Il vient ï¿½ votre rencontre, vous commencez ï¿½ discuter.\n");
        printf("Il vous explique qu'il connait trï¿½s bien cette map, et qu'il pourrait vous aider ï¿½ vous mettre en sï¿½curitï¿½ dans un lieu discret.\n");
        printf("Vous ï¿½tes trï¿½s intï¿½ressï¿½ par l'aide qu'il vous propose, et il l'a trï¿½s bien compris. Il vous propose un marchï¿½ :\n");
        printf("\"J'ai besoin de trouver l'item %s, si tu acceptes d'aller me chercher cet item et de me la ramener par la suite, je t'aiderais ï¿½ te sortir d'ici.\"\n", quete->recherche.wanted.name);

        do{
            printf("Vous avez deux possibilites :\n");
            printf("1 - Accepter la requete de l'homme\n");
            printf("2 - Ne pas l'accepter\n");
            printf("Que choisissez vous ?\n");
            scanf("%d", &choix);

            if(choix<1 || choix>2)
                printf("Valeur incorrecte. Veuillez resaissir.\n");
        }while(choix<1 || choix>2);

        /*Le joueur souhaite trouver l'item*/
        if(choix){
            printf("L'homme est heureux de votre rï¿½ponse, il vous donne plus de dï¿½tails sur votre mission :\n");
            printf("\"Tu as le choix d'aller dans une ville, dans un marchet ou dans une favella pour trouver l'item %s. Une fois que tu as trouve l'item il faudra que tu revienne ici pour me le redonner. ", quete->recherche.wanted.name);
            printf("Bien entendu si ton inventaire est plein, tu devras ï¿½changer l'item que je te demande contre un en ta possession.\"\n");

            printf("A ce moment prï¿½sent ton inventaire contient %d items sur %d items possible.\n", player->nb_items_inventory, INVENTORY_CAPACITY);
            do{
                printf("Souhaitez-vous toujours trouver l'item demandï¿½ par l'homme ?\n");
                printf("1 - Oui\n");
                printf("2 - Non\n");
                printf("Votre choix : ");
                scanf("%d", &conf);

                if(conf<1 || conf>2)
                    printf("Valeur incorrecte. Veuillez resaissir.\n");
            }while(conf<1 || conf>2);

            /*Le joueur renonce ï¿½ la requete*/
            if(conf==2){
                printf("Vous lui annoncez renoncer ï¿½ cette requï¿½te, il es deï¿½u mais vous laisse repartir explorer la map.\n");
                return 0;
            }
            /*Le joueur continue la requete*/
            else{
                printf("C'est parti, a vous de trouver l'item %s que l'homme vous demande !\n", quete->recherche.wanted.name);
                quete->recherche.trouve=0;
                return 0;
            }
        }
        /*Le joueur ne souhaite pas trouver l'item demandï¿½ par l'homme*/
        else{
            printf("Vous lui annoncez ne pas souhaitez honorer cette requï¿½te, il es deï¿½u mais vous laisse repartir explorer la map.\n");
            return 0;
        }
        return (-1);
    }

    /*Si la quete est dï¿½ja commencï¿½ par le joueur, et qu'il est entrain de chercher l'item, il souhaite rechercher les items sur l'hexagone actuel*/
    else if(quete->recherche.trouve==0){
        /*Le joueur n'est pas sur un hexagone de categorie urbain*/
        if(map[player->posX][player->posY].categ != urbain){
            printf("Vous ne pourrez pas trouver l'item %s recherchï¿½ par l'homme ici, vous n'ï¿½tes ni en ville, ni au marchet et pas dans une favella.\n", quete->recherche.wanted.name);
            printf("Continuez votre route sur la map, afin d'aller dans un des lieux sitï¿½s ci-dessus.\n");
            return 0;
        }
        /*Le joueur est sur un hexagone de categorie urbain*/
        else{
            printf("Vous ï¿½tes dans un lieu oï¿½ vous pouvez trouver l'item %s recherchï¿½ par l'homme !\n", quete->recherche.wanted.name);
            /*Recherche d'items et verification si item trouve par le joueur*/
            scavenge(map,player,Tab_Items,nb_items_available);
            /*L'item a ï¿½tï¿½ trouvï¿½ par le joueur sur l'hexagone*/
            if(item_in_inventory(*player,quete->recherche.wanted.name) != -1){
                printf("Fï¿½licitations, vous avez trouvï¿½ l'item %s que l'homme vous a demande ! Il faut maintenant aller le retrouver pour lui donner.\n", quete->recherche.wanted.name);
                quete->recherche.trouve=1;
                return 0;
            }
            /*L'item n'a pas ï¿½tï¿½ trouvï¿½ par le joueur sur l'hexagone, ou ce dernier n'a pas souhaitez l'ajouter ï¿½ son inventaire*/
            else{
                printf("Vous n'avez pas trouvï¿½ l'item recherchï¿½ %s, cela sera pour une prochaine fois dans un autre lieu.\n", quete->recherche.wanted.name);
                return 0;
            }
        }
        return (-1);
    }

    /*Le joueur a trouvï¿½ par le joueur, il doit maintenant le donner ï¿½ l'homme*/
    else if(quete->recherche.trouve==1){
        /*Le joueur n'est pas sur le bon hexagone pour retrouver l'homme*/

        // if(map[player->posX][player->posY] != map[quest_map[5][0]][quest_map[5][1]])
        // Changement car pb lors de la compilation - DEMANDER A ANAIS POUR CONFIRMATION !
        if((player->posX != quest_map[5][0]) && (player->posY != quest_map[5][1])){
            printf("Vous ne pourrez pas retrouver l'homme ici, vous n'ï¿½tes pas sur ce lieu de votre rencontre.\n");
            printf("Continuez votre route sur la map, afin de retouver l'homme.\n");
            return 0;
        }
        /*Le joueur n'est pas sur le bon hexagone pour retrouver l'homme*/
        else{
            printf("Vous ï¿½tes dans le lieu oï¿½ vous avez rencontrï¿½ l'homme !\n");
            printf("Il se tient face a vous avec un grand sourire. Vous lui donnez son item, il est au passage retirï¿½ de votre inventaire. Il vous remercie ï¿½normï¿½ment de lui avoi rendu ce service.\n");
            delete_item_in_inventory(player, quete->recherche.wanted);
            printf("Pour vous remercier, et comme il vous l'avez promis il vous indique le lieu oï¿½ vous pourrez vous rï¿½fugier et ainsi vous mettre en sï¿½curitï¿½.\n");
            printf("Voilï¿½ les coordonnï¿½es : X=%d, Y=%d\n", quete->recherche.bunkerX, quete->recherche.bunkerY);
            quete->recherche.situation=1;
            return 0;
        }
    }
    return (-1);
}
=======
 * \return Retourne un \a int : 0 si le jeu continue et -1 si problème dans la quete.
*/
int quete_recherche(perso_t * player, cell_t map[D][D], quete_t * quete, quest_map[6][2], item_t * Tab_Items, int nb_items_available){
    int nb_items_urban, num, choix, conf;

    /*Si la quete n'est pas encore commencé, ou que le joueur y a deja renoncé*/
    if(quete->recherche.trouve==-1){
        quete->recherche.situation=0;

        nb_items_urbain = compter_items_urbain(Tab_Items, nb_items_available); //Nombre d'item que l'on peut trouver dans les hexagones de categorie urbain
        /*Création + initialisation d'un tableau contenant les items que l'on peux trouver dans un haxagone de categorie urbain*/
        item_t Tab_Items_urbain = malloc(nb_items_urbain * sizeof(item_t));
        init_Tab_Items_urbain(&Tab_Items_urbain, Tab_Items, nb_items_urbain);

        /*Generation aleatoire de l'item à trouver*/
        num = rand()%nb_items_urbain;
        quete->recherche.wanted = Tab_Items_urbain[num];

        /*Description de la situation*/
        printf("Vous croiser la chemin d'un homme qui paraît sympathique. Il vient à votre rencontre, vous commencez à discuter.\n");
        printf("Il vous explique qu'il connait très bien cette map, et qu'il pourrait vous aider à vous mettre en sécurité dans un lieu discret.\n");
        printf("Vous êtes très intéressé par l'aide qu'il vous propose, et il l'a très bien compris. Il vous propose un marché :\n");
        printf("\"J'ai besoin de trouver l'item %s, si tu acceptes d'aller me chercher cet item et de me la ramener par la suite, je t'aiderais à te sortir d'ici.\"\n", quete->recherche.wanted.name);

        do{
            printf("Vous avez deux possibilites :\n");
            printf("1 - Accepter la requete de l'homme\n");
            printf("2 - Ne pas l'accepter\n");
            printf("Que choisissez vous ?\n");
            scanf("%d", &choix);

            if(choix<1 || choix>2)
                printf("Valeur incorrecte. Veuillez resaissir.\n");
        }while(choix<1 || choix>2);

        /*Le joueur souhaite trouver l'item*/
        if(choix){
            printf("L'homme est heureux de votre réponse, il vous donne plus de détails sur votre mission :\n");
            printf("\"Tu as le choix d'aller dans une ville, dans un marchet ou dans une favella pour trouver l'item %s. Une fois que tu as trouve l'item il faudra que tu revienne ici pour me le redonner. ", quete->recherche.wanted.name);
            printf("Bien entendu si ton inventaire est plein, tu devras échanger l'item que je te demande contre un en ta possession.\"\n");

            printf("A ce moment présent ton inventaire contient %d items sur %d items possible.\n", player->nb_items_inventory, INVENTORY_CAPACITY);
            do{
                printf("Souhaitez-vous toujours trouver l'item demandé par l'homme ?\n");
                printf("1 - Oui\n");
                printf("2 - Non\n");
                printf("Votre choix : ");
                scanf("%d", &conf);

                if(conf<1 || conf>2)
                    printf("Valeur incorrecte. Veuillez resaissir.\n");
            }while(conf<1 || conf>2);

            /*Le joueur renonce à la requete*/
            if(conf==2){
                printf("Vous lui annoncez renoncer à cette requête, il es deçu mais vous laisse repartir explorer la map.\n");
                return 0;
            }
            /*Le joueur continue la requete*/
            else{
                printf("C'est parti, a vous de trouver l'item %s que l'homme vous demande !\n", quete->recherche.wanted.name);
                quete->recherche.trouve=0;
                return 0;
            }
        }
        /*Le joueur ne souhaite pas trouver l'item demandé par l'homme*/
        else{
            printf("Vous lui annoncez ne pas souhaitez honorer cette requête, il es deçu mais vous laisse repartir explorer la map.\n");
            return 0;
        }
        return (-1);
    }

    /*Si la quete est déja commencé par le joueur, et qu'il est entrain de chercher l'item, il souhaite rechercher les items sur l'hexagone actuel*/
    else if(quete->recherche.trouve==0){
        /*Le joueur n'est pas sur un hexagone de categorie urbain*/
        if(map[player->posX][player->posY].categ != urbain){
            printf("Vous ne pourrez pas trouver l'item %s recherché par l'homme ici, vous n'êtes ni en ville, ni au marchet et pas dans une favella.\n", quete->recherche.wanted.name);
            printf("Continuez votre route sur la map, afin d'aller dans un des lieux sités ci-dessus.\n");
            return 0;
        }
        /*Le joueur est sur un hexagone de categorie urbain*/
        else{
            printf("Vous êtes dans un lieu où vous pouvez trouver l'item %s recherché par l'homme !\n", quete->recherche.wanted.name);
            /*Recherche d'items et verification si item trouve par le joueur*/
            scavenge(map,&player,Tab_Items,nb_items_available);
            /*L'item a été trouvé par le joueur sur l'hexagone*/
            if(item_in_inventory(*player,quete->recherche.wanted.name) != -1){
                printf("Félicitations, vous avez trouvé l'item %s que l'homme vous a demande ! Il faut maintenant aller le retrouver pour lui donner.\n", quete->recherche.wanted.name);
                quete->recherche.trouve=1;
                return 0;
            }
            /*L'item n'a pas été trouvé par le joueur sur l'hexagone, ou ce dernier n'a pas souhaitez l'ajouter à son inventaire*/
            else{
                printf("Vous n'avez pas trouvé l'item recherché %s, cela sera pour une prochaine fois dans un autre lieu.\n", quete->recherche.wanted.name);
                return 0;
            }
        }
        return (-1);
    }

    /*Le joueur a trouvé par le joueur, il doit maintenant le donner à l'homme*/
    else if(quete->recherche.trouve==1){
        /*Le joueur n'est pas sur le bon hexagone pour retrouver l'homme*/
        if(map[player->posX][player->posY] != map[quest_map[5][0]][quest_map[5][1]]){
            printf("Vous ne pourrez pas retrouver l'homme ici, vous n'êtes pas sur ce lieu de votre rencontre.\n");
            printf("Continuez votre route sur la map, afin de retouver l'homme.\n");
            return 0;
        }
        /*Le joueur n'est pas sur le bon hexagone pour retrouver l'homme*/
        else{
            printf("Vous êtes dans le lieu où vous avez rencontré l'homme !\n");
            printf("Il se tient face a vous avec un grand sourire. Vous lui donnez son item, il est au passage retiré de votre inventaire. Il vous remercie énormément de lui avoi rendu ce service.\n");
            delete_item_in_inventory(player, quete->recherche.wanted);
            printf("Pour vous remercier, et comme il vous l'avez promis il vous indique le lieu où vous pourrez vous réfugier et ainsi vous mettre en sécurité.\n");
            printf("Voilà les coordonnées : X=%d, Y=%d\n", quete->recherche.bunkerX, quete->recherche.bunkerY);
            quete->recherche.situation=1;
            return 0;
        }
    }
    return (-1);
}
>>>>>>> Stashed changes
